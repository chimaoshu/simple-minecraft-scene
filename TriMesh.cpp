...
TriMesh::TriMesh(bool needRotate)
{
...
// 传入Base矩阵，计算并设置modelMatrix并返回
glm::mat4 TriMesh::calculateAndSetModelMatrixByBase(
	const glm::mat4 base,
	PointfRotation rotation_point)
{
	glm::mat4 model = glm::mat4(1.0f);

	// 围绕的旋转点
	glm::vec3 rotation_point_pos;
	if (rotation_point == PointfRotation::ZeroPoint)
	{
		rotation_point_pos = glm::vec3(0.0, 0.0, 0.0);
	}
	else if (rotation_point == PointfRotation::UpFaceCenter)
	{
		rotation_point_pos = glm::vec3((max_x + min_x) / 2, max_y, (max_z + min_z) / 2);
	}

	// 4-做完变换后从原点移动回原来位置
	model = glm::translate(model, rotation_point_pos);
	// 3
...
void TriMesh::storeFacesPoints()
{
	...
	for (int i = 0; i < faces.size(); i++)
	{
		...
		this->points.push_back(this->vertex_positions[x]);
		this->points.push_back(this->vertex_positions[y]);
		this->points.push_back(this->vertex_positions[z]);

		// 计算最大最小的x与y
		std::vector<int> v{x, y, z};
		for (int j : v)
		{
			auto &position = vertex_positions[j];
			if (position.x > max_x)
				max_x = position.x;
			if (position.y > max_y)
				max_y = position.y;
		...
		// 颜色
		x = color_index[i].x, y = color_index[i].y, z = color_index[i].z;
		this->colors.push_back(this->vertex_colors[x]);
		this->colors.push_back(this->vertex_colors[y]);
		this->colors.push_back(this->vertex_colors[z]);
		// 法向量
		x = normal_index[i].x, y = normal_index[i].y, z = normal_index[i].z;
		this->normals.push_back(this->vertex_normals[x]);
		this->normals.push_back(this->vertex_normals[y]);
		this->normals.push_back(this->vertex_normals[z]);
		// 纹理
		x = texture_index[i].x, y = texture_index[i].y, z = texture_index[i].z;
		this->textures.push_back(this->vertex_textures[x]);
		this->textures.push_back(this->vertex_textures[y]);
		this->textures.push_back(this->vertex_textures[z]);
	}
...
void TriMesh::generateSquare(int block_num)
{
	// 创建顶点前要先把那些vector清空
	cleanData();
	...
	for (int i = 0; i < block_num; i++)
	{
		for (int j = 0; j < block_num; j++)
		{
			...
			vertex_positions.push_back(glm::vec3(i + 0, 0, j + 0));
			vertex_positions.push_back(glm::vec3(i + 1, 0, j + 0));
			vertex_positions.push_back(glm::vec3(i + 1, 0, j + 1));
			vertex_positions.push_back(glm::vec3(i + 0, 0, j + 1));

			// 每个三角面片的顶点下标
			int base = 4 * (i * block_num + j);
			faces.push_back(vec3i(base, base + 1, base + 2));
...
void TriMesh::readObj(const std::string &filename, const std::string partName)
{
	...
	while (std::getline(fin, line))
	{
		...
		{
			sin >> _x >> _y >> _z;
			this->vertex_positions.push_back(glm::vec3(_x, _y, _z));
			continue;
		}
		// vertex_normals
		else if (type == "vn")
		{
			sin >> _x >> _y >> _z;
			this->vertex_normals.push_back(glm::vec3(_x, _y, _z));
			this->vertex_colors.push_back(glm::vec3(_x, _y, _z)); // 颜色
			continue;
		}
		// vertex_textures
		else if (type == "vt")
		{
			sin >> _x >> _y;
			this->vertex_textures.push_back(glm::vec2(_x, _y));
			continue;
		}
		// faces
		else if (type == "f")
		{
			// 节点照样都，part未开始则跳过face的读取
			if (!part_is_begin)
				continue;

			sin >>
				a0 >> slash >> b0 >> slash >> c0 >>
				a1 >> slash >> b1 >> slash >> c1 >>
				a2 >> slash >> b2 >> slash >> c2;

			// positions
			this->faces.push_back(vec3i(a0 - 1, a1 - 1, a2 - 1));
			// texture_index
			this->texture_index.push_back(vec3i(b0 - 1, b1 - 1, b2 - 1));
			// normal_index
			this->normal_index.push_back(vec3i(c0 - 1, c1 - 1, c2 - 1));
			// 其中vertex_color和color_index可以用法向量的数值赋值
			this->color_index.push_back(vec3i(c0 - 1, c1 - 1, c2 - 1));
		}
		else
		{
			continue;
