...
public:
	...
	// 读取天空盒的纹理文件
	void load_cube_map(const std::vector<std::string> faces, GLuint &texture);

	// addMesh天空和专属
	void addSkyCube(TriMesh *mesh, const std::string &name, const std::vector<std::string> &faces, const std::string &vshader, const std::string &fshader);
	...
	// void bindObjectAndData(TriMesh *mesh, openGLObject &object, const std::vector<std::string> &faces, const std::string &vshader, const std::string &fshader);
