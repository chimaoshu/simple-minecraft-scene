...
enum PointfRotation
{
	ZeroPoint,	  // 原点
	UpFaceCenter, // 上表面中点
				  // DownFaceCenter, // 下表面中点
};

class TriMesh
{
public:
	TriMesh(bool needRotate = false);
	~TriMesh();
	...
	void readObj(const std::string &filename, const std::string partName);
	...
	glm::mat4 calculateAndSetModelMatrixByBase(
		const glm::mat4 base,
		PointfRotation rotation_point = PointfRotation::ZeroPoint);

	float height = -1; // 最大最小y的差值
	float width = -1;  // 最大最小x的差值
	float max_x;
	float max_y;
	float max_z;
	float min_x;
...
protected:
	...
	glm::mat4 modelMatrix = glm::mat4(1.0f); // 模型变化矩阵
};
